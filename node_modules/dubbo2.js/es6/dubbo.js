...
const log = debug_1.default('dubbo:bootstrap');
const packageVersion = require('../package.json').version;
log('dubbo2.js version :=> %s', packageVersion);
//定位没有处理的promise
process.on('unhandledRejection', (reason, p) => {
    log(reason, p);
    util_1.traceErr(new Error(reason));
});
/**
 * Dubbo
 *
...
class Dubbo {
    constructor(props) {
        ...
        this.proxyService = (provider) => {
            ...
            Object.keys(methods).forEach(name => {
                proxyObj[name] = (...args) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    const middlewares = [
                        ...
                        function handleRequest(ctx) {
                            return __awaiter(this, void 0, void 0, function* () {
                                ...
                                ctx.body = yield go_1.go(self._queue.add(ctx));
                                log('end handle dubbo request');
                            });
                        },
                    ];
                    log('middleware->', middlewares);
                    const fn = koa_compose_1.default(middlewares);
                    try {
                        yield fn(ctx);
                    }
                    catch (err) {
        ...
        this._readyResolve = util_1.noop;
        this._subscriber = {
            onTrace: util_1.noop,
        };
        //初始化消息监听
    ...
    _initMsgListener() {
        process.nextTick(() => {
            util_1.msg
                .addListener('sys:trace', (msg) => {
                this._subscriber.onTrace(msg);
            })
                .addListener('sys:ready', () => {
                this._readyResolve();
            });
            util_1.traceInfo(`dubbo:bootstrap version => ${packageVersion}`);
        });
    }
    /**
     * 注册服务到dubbo容器中
