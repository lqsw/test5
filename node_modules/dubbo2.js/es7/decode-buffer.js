...
const log = debug_1.default('dubbo:decode-buffer');
/**
    ...
    receive(data) {
        ...
        while (bufferLength >= HEADER_LENGTH) {
            ...
            if (magicHigh != MAGIC_HIGH || magicLow != MAGIC_LOW) {
                log(this._buffer);
                log(`receive server data error, buffer[0] is 0xda ${magicHigh ==
            ...
            if (magicHigh === MAGIC_HIGH && magicLow === MAGIC_LOW) {
                ...
                const bodyLength = byte_1.fromBytes4(bodyLengthBuff);
                log('body length', bodyLength);
                //判断是不是心跳
                if (heartbeat_1.default.isHeartBeat(header)) {
